...
const loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.isTwoFactorEnabled) {
    ...
    await sendEmail({
      email: user.email,
      subject: "MovieHaat Login OTP Code",
      message,
    });
...
const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: `MovieHaat Password Recovery`,
      message,
    });
...
const updateProfile = catchAsyncErrors(async (req, res, next) => {
  ...
  if (avatarFile) {
    if (!avatarFile.mimetype?.startsWith("image/")) {
      return next(new ErrorHandler("Avatar must be an image", 400));
    }
    const oldAvatarKey = user.avatar?.public_id;
    const uploaded = await uploadToS3(avatarFile, "users/avatars");
    newUserData.avatar = { public_id: uploaded.key, url: uploaded.url };
    if (oldAvatarKey) await deleteFromS3(oldAvatarKey);
  }
