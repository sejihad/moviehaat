...
const Blogs = () => {
  const dispatch = useDispatch(); const { blogs, loading, error } = useSelector((state) => state.blogs);
  useEffect(() => { dispatch(getBlog()); }, [dispatch]);
  const list = (blogs || []).map(view); const [featured, ...articles] = list;
