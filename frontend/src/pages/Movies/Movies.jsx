...
const Movies = () => {
  ...
  </div></section><section className="container movies-library"><header className="movies-library__heading"><div><p>Explore our collection</p><h1>{activeCategory}</h1></div><span>{visibleMovies.length} movies</span></header>
    {loading && !movies.length ? <div className="movie-state">Loading movies...</div> : error && !movies.length ? <div className="movie-state">{error}</div> : !visibleMovies.length ? <div className="movie-state">No movies found in this category.</div> : <div className="movies-grid">{visibleMovies.map((movie) => <Link to={`/movies/${movie.id}`} className="library-movie-card" key={movie.id}><div className="library-movie-card__poster"><img src={movie.image} alt={`${movie.title} poster`} loading="lazy"/><span className="library-movie-card__rating"><Star size={12} fill="currentColor"/> {movie.rating}</span><div className="library-movie-card__overlay"><span><Play size={21} fill="currentColor"/></span></div></div><h2>{movie.title}</h2></Link>)}</div>}
  </section></main>;
};
